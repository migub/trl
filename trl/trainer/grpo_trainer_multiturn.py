...
class GRPOTrainer(_BaseTrainer):
    ...
    def __init__(
        ...
        # MULTITURN: Additional parameters for multi-turn negotiation
        ...
        # MULTITURN: Store multi-turn negotiation config
    ...
    # ============================================================
    # MULTITURN: Multi-Turn Dialogue Generation Methods
    # ============================================================

    @torch.no_grad()
    def _multiturn_generate_single_response(self, messages: list) -> str:
        """Generate a single response given a conversation history (for multi-turn negotiation)."""
        ...
        for round_num in range(self.max_negotiation_rounds):
            ...
            for speaker in speakers:
                if speaker == "agent":
                    ...
                    response = self._multiturn_generate_single_response(agent_history)
    ...
    def _tokenize_multiturn_conversation(
        ...
        Returns:
            prompt_ids: List of token IDs for the system prompt
            completion_ids: List of token IDs for the conversation (all turns)
            assistant_mask: List of 0/1 values (1 = agent token, 0 = opponent token)
        """
        ...
        # Build assistant_mask: 1 = agent token, 0 = opponent/format token
        ...
        # Fallback: if no agent tokens found, mark ALL as agent to prevent NaN loss
        if sum(assistant_mask) == 0:
            logger.warning(
                f"No agent tokens detected in assistant_mask. "
                f"Falling back to full completion mask. "
                f"Conversation had {len(conversation)} messages, "
                f"agent_turn_indices={agent_turn_indices}"
            )
            assistant_mask = [1] * len(completion_ids)

        return prompt_ids, completion_ids, assistant_mask

    def _generate_multiturn(self, inputs: list[dict]) -> tuple:
        """
        Multi-turn generation: play negotiation dialogues and return results
        in the same format as _generate() for seamless integration.

        Returns same tuple as _generate():
            (prompt_ids, completion_ids, tool_mask, completions, total_tokens, logprobs, extra_fields)
        """
        device = self.accelerator.device
        mode = "train" if self.model.training else "eval"
        ...
        for inp in inputs:
            ...
            if isinstance(agent_prompt, list):
                # Conversational format: extract the text content
        ...
        # The assistant_mask serves as tool_mask: 1 = agent (train), 0 = opponent (don't train)
        tool_mask_list = all_assistant_masks

        # Decode completions for reward computation
        completions = []
        for conv in completions_for_reward:
            completions.append(conv)  # Pass full conversation to reward function
        ...
        if mode == "train":
            total_tokens = (agg_prompt_lengths.sum() + agg_completion_lengths.sum()).item()
        ...
        # Log agent token ratio
        total_completion = sum(len(c) for c in all_completion_ids)
        total_agent = sum(sum(m) for m in all_assistant_masks)
        agent_ratio = total_agent / max(total_completion, 1)
        self._metrics[mode]["multiturn/agent_token_ratio"].append(agent_ratio)
        ...
        return (
            ...
            None,  # logprobs (not used)
            {},    # extra_fields
        )

    # ============================================================
    # END MULTITURN methods
    # ============================================================
    ...
    def _generate(self, prompts: list, inputs: list | None = None):
        device = self.accelerator.device
        mode = "train" if self.model.training else "eval"
        ...
        # MULTITURN: Use multi-turn generation if enabled
        if self.multiturn and inputs is not None:
            return self._generate_multiturn(inputs)
    ...
    def _generate_and_score_completions(
        ...
        (
            ...
            extra_fields,
        ) = self._generate(prompts, inputs=inputs)  # MULTITURN: pass inputs for multi-turn generation
